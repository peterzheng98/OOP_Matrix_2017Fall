    ...
    public:
        ...
        void resize(size_t _n, size_t _m, T _init = T()) {
            ...
            for (int i = 0; i < _n; i++) {
                ...
                _matrix[i].resize(_col);
                for (int j = 0; j < _m; j++) {
                    if (i >= _row) _matrix[i][j] = _init;
                    else if (j >= _col) _matrix[i][j] = _init;
                    else _matrix[i][j] = _matrix[i][j];
                }
...
//                    __matrix[i].resize(_col);
                    for (int j = 0; j < _col; j++) {
                        this->_matrix[i][j] = this->_matrix[i][j] + o(i, j);
                    }
                    // __matrix.push_back(p);
                }
...
//            std::vector<std::vector<decltype(T() * U())> > __matrix;
//            __matrix.resize(this->_row);
            for (int i = 0; i < this->_row; i++) {
                // std::vector<decltype(T() * U())> d;
//                __matrix[i].resize(_col);
                for (int j = 0; j < _col; j++) {
                    this->_matrix[i][j] = this->_matrix[i][j] * x;
                }
